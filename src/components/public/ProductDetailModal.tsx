...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 sm:p-6">
      <div className="relative w-full max-w-xl md:max-w-2xl bg-white rounded-3xl shadow-2xl border border-zinc-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
            <div className="aspect-square rounded-2xl bg-zinc-50 p-6 flex items-center justify-center border border-zinc-100">
              <img
                src={product.image}
                alt={product.name}
            ...
            <div className="space-y-2 pt-4 border-t border-zinc-100">
              ...
              <div className="flex flex-wrap gap-1.5">
                {product.benefits.map((b, idx) => (
                  <span
                    key={idx}
                    className="px-2.5 py-1 rounded-md bg-zinc-100 text-[11px] text-zinc-700 font-mono"
                  >
                    {b}
                  </span>
                ))}
        ...
        <div className="p-6 border-t border-zinc-100 flex items-center justify-between">
          <button
            onClick={onClose}
            className="px-5 py-2.5 rounded-full border border-zinc-200 text-zinc-700 text-xs font-semibold uppercase tracking-wider hover:bg-zinc-50 transition-colors cursor-pointer"
          >
            Tutup
          </button>
