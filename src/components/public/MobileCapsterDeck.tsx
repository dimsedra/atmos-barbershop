...
const AUTO_SWIPE_INTERVAL = 8000;
...
export const MobileCapsterDeck: React.FC<MobileCapsterDeckProps> = ({ onBookWithCapster }) => {
  ...
  const total = CAPSTERS_DATA.length;
  const currentStylist = CAPSTERS_DATA[currentIndex];
  const nextStylist = CAPSTERS_DATA[(currentIndex + 1) % total];
  const nextNextStylist = CAPSTERS_DATA[(currentIndex + 2) % total];
  ...
  return (
    <div className="space-y-6 select-none touch-none">
      {/* Top Multi-Story Progress Bar */}
      <div className="space-y-2 max-w-md mx-auto">
        <div className="flex items-center gap-1.5 px-1">
          {CAPSTERS_DATA.map((stylist, idx) => {
      ...
      <div className="relative h-[510px] sm:h-[560px] max-w-md mx-auto">
        {/* Card 3 */}
        <div
