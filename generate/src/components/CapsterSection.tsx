...
export const CapsterSection: React.FC<CapsterSectionProps> = ({
  ...
  const [selectedId, setSelectedId] = useState<string>(STYLISTS_DATA[0].id);
  const cardRefs = useRef<{ [key: string]: HTMLDivElement | null }>({});
  const isClickingRef = useRef<boolean>(false);

  const activeStylist = STYLISTS_DATA.find((s) => s.id === selectedId) || STYLISTS_DATA[0];
  ...
  // Scroll sync: As each capster card passes through the focal viewport reading area, sync the left spotlight
  useEffect(() => {
  ...
  return (
    <section id="capster" className="py-20 md:py-24 xl:py-28 bg-[#F8F7F4] border-b border-[#E6E4DF]">
      <div className="max-w-7xl 2xl:max-w-[1720px] mx-auto px-6 sm:px-10 lg:px-12 xl:px-16 space-y-12 lg:space-y-14">
        ...
        {/* Mobile & Tablet Dedicated Experience: Tinder-style Infinite Swipe Deck (8s auto-swipe) */}
        <div className="block lg:hidden">
          <MobileCapsterDeck onBookWithCapster={onBookWithCapster} />
        </div>
        ...
        <div className="hidden lg:grid lg:grid-cols-12 gap-8 lg:gap-12 items-start relative">
          ...
          {/* Left: Active Stylist Portrait Stage (5 cols) - Sticky Pinned with Substantial Full Height */}
          <div className="lg:col-span-5 lg:sticky lg:top-24 space-y-4">
            
            {/* Full-bodied Portrait Card */}
            <div className="relative rounded-3xl overflow-hidden h-[460px] sm:h-[500px] xl:h-[540px] w-full bg-zinc-900 border border-[#E6E4DF] shadow-[0_8px_30px_rgba(0,0,0,0.06)]">
              ...
              {/* Gradient Scrim for Impeccable Text Contrast */}
            ...
            {/* Direct Booking Action for Active Stylist */}
            <div className="space-y-2">
              <button
                onClick={() => onBookWithCapster(activeStylist.branchId, activeStylist)}
          ...
          {/* Right: Interactive Editorial Roster (7 cols) - Balanced Vertical Flow */}
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="space-y-2 flex-1">
                      ...
                      {/* Specialties in clean scannable text */}
